   ...
   public  class ExpanseHeadCls
    {
        ...
        public ExpanseHeadCls()
        { }

        public ExpanseHeadCls(DataRow dr)
        {
            if (dr.Table.Columns.Contains("Id") && !Convert.IsDBNull(dr["Id"]))
                this.Id = Convert.ToInt32(dr["Id"]);

            if (dr.Table.Columns.Contains("ExpanseHead") && !Convert.IsDBNull(dr["ExpanseHead"]))
                this.ExpanseHead = Convert.ToString(dr["ExpanseHead"]);
