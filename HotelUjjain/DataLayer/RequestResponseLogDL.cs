...
namespace DataLayer
{
    public class RequestResponseLogDL
    {
        private int idUsr
        {
            get
        ...
        public ResponseDto GetRequestResponseLog(string Con = "")
        {
            ...
            try
            {
                ...
                List<RequestResponseLogDto> lstUserDetails = new List<RequestResponseLogDto>();
                DataSet ds = new DataSet();
                SqlParameter[] parameter = new SqlParameter[0];
                string ResponseMessage = string.Empty;
                int ResponseCode = 0;
                //SqlHelper.ELHelper elHelper = new SqlHelper.ELHelper();
                ds = SqlHelper.ExecuteDataset(Con, "usp_GetRequestResponseLog", parameter);
                if (ds != null)
                {
                    ...
                    if (ds.Tables.Count > 1)
                    {
                        ...
                        {
                            lstUserDetails.Add(new RequestResponseLogDto(dr));
                        }
                    }
                }
        ...
        public ResponseDto InsertUpdateDeleteRequestResponseLog(RequestResponseLogDto para, string Con = "")
        {
            if (string.IsNullOrEmpty(Con))
                Con = CommonDL.conString;
            try
